...
class DynamoModel:
    ...
    def __init__(self, **kwargs):
        ...
        if kwargs:
            schema = self._dynamo_table.get_schema()
            schema_json = schema.get_schema_json() if schema else {}
            if schema:
                schema.fill_defaults(kwargs)
                schema.validate(kwargs)

            for key, value in kwargs.items():
                if key in schema_json:
        ...
        def wrapper(self, *args, **kwargs):
            if not self._dynamo_table:
                raise ValueError(
                    "DynamoTable instance is not set. Call set_dynamo_table with a DynamoTable instance.")
            key = self.fetch_key()
            original_version = self._dynamo_table.get(key).get(
